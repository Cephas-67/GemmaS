...
// Plan WebGL plein écran avec un motif animé de "soie" + bruit fractal.
...
const SilkPlane = forwardRef(function SilkPlane(
  ...
  // Quand `paused` est vrai (section hors viewport) on garde la dernière
  // frame affichée mais on n'avance plus le temps : zéro travail GPU utile,
  // et surtout aucun flash au retour (le canvas n'est jamais démonté).
  useFrame((_, delta) => {
    if (paused) return;
    const mesh = (ref as React.MutableRefObject<Mesh | null>).current;
    if (!mesh) return;
...
export function Silk({
  ...
  // dpr cap à 1 : le shader est volontairement flou (noise + sinus), rendre
  // au-delà du logical-pixel ne se voit pas et coûte 2-4x plus cher.
  return (
